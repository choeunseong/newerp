...
public interface MemberService {
	...
	List<MemberVO> emplTabSearch(MemberVO vo);	// 직원목록조회
	int emplCount(MemberVO vo);	// 직원목록조회 count

	List<CommVO> selectCompany();	// 본지사 조회

	int memberInsert(MemberVO vo);	// 회원가입 하기
	...
	int memberDel(MemberVO vo);	// 직원/강사 정보 삭제
	int emplUpdate(MemberVO vo);	// 직원정보 수정
	
	List<MemberVO> teacherTabSearch(MemberVO vo);	// 강사 목록조회
